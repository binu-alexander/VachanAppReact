  ...
  _renderHeader = (item, expanded) => {
    return (
      <View style={styles.headerContainer}>
        ...
        <Icon style={styles.iconStyle} name={"keyboard-arrow-down"} size={24} />
      </View>
    )
  }
  _renderContent = (item) => {
      ...
      <View>
        ...
        {item.versionModels.map((element, index, key) => (
          <TouchableOpacity
            ...
            <View>
              <Text style={[styles.text, { marginLeft: 8, fontWeight: 'bold' }]} >{element.versionCode} </Text>
              <Text style={[styles.text, { marginLeft: 8 }]} >{element.versionName}</Text>
            </View>
            <View style={{ padding: 20 }}>
              {
                element.downloaded === true ?
                  item.languageName.toLowerCase() === 'english' ? null :
                    <Icon style={[styles.iconStyle, { marginRight: 8 }]} name="delete" size={24} onPress={() => { this.deleteBible(item.languageName, item.languageCode, element.versionCode, element.sourceId, element.downloaded) }}
                    />
                  :
                  item.languageName.toLowerCase() === 'english' ? null :
                    <Icon style={[styles.iconStyle, { marginRight: 12 }]} name="file-download" size={24} onPress={() => { this.downloadBible(item.languageName, element.versionCode, item.bookNameList, element.sourceId) }} />
              }
            </View>
          </TouchableOpacity>
        ))}
