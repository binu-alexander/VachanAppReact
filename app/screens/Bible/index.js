...
import { APIAudioURL, fetchVersionBooks, userInfo, updateVersionBook, updateVerseNumber, updateFontSize, updateVersion, updateMetadata } from "../../store/action/";
...
const Bible = (props) => {
  const { language, languageCode, versionCode, sourceId, downloaded, contentType, baseAPI, chapterNumber, bookName, bookId, sizeFile, colorFile, books, visibleParallelView } = props
  ...
  const getChapter = async () => {
    try {
      if (downloaded) {
        ...
        if (baseAPI != null) {
          var content = await vApi.get("bibles" + "/" + sourceId + "/" + "books" + "/" + bookId + "/" + "chapter" + "/" + currentVisibleChapter);
          if (content) {
  ...
  const getReference = async (item) => {
    ...
    if (item) {
      ...
      props.updateVersionBook({
        bookId: item.bookId, bookName: item.bookName, chapterNumber: parseInt(item.chapterNumber), totalChapters: item.totalChapters,
      });
  ...
  }, [language, sourceId, baseAPI, visibleParallelView, currentVisibleChapter, bookId]);
  return (
    <BibleContext.Provider value={[{
      audio, clampedScroll, _clampedScrollValue, showBottomBar,
      ...
      styles, status, chapterHeader, scrollAnim, offsetAnim, unAvailableContent, isLoading,
